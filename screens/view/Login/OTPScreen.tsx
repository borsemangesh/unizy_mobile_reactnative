...
const handleChange = (text: string, index: number) => {
  ...
  const unsubscribe = navigation.addListener('focus', clearOtp);
  clearOtp();
  return unsubscribe;
}, [navigation]);
...
const handlesignup = () =>{
  ...
  useEffect(() => {
    const fetchEmail = async () => {
      try {
        const storedEmail = await AsyncStorage.getItem('personal_mail_id');
        if (storedEmail) {
          setPersonalEmail(storedEmail);
        }
      } catch (err) {
...
const handleresend = async () => {
  ...
  try {
    ...
    //const response= await postRequest('user/user-signup',{temp_user_id: Number(tempUserId),})
...
//   ]}
// >
 <Animated.View style={[styles.formContainer, { height: 'auto' }]}>
<Animated.View
         style={[
        { width: '100%', alignItems: 'center' },
        { transform: [{ translateY: slideAnim }], opacity },
        ...
        <View style={styles.privacyContainer}>
          <Text style={styles.termsText}>
          We have sent a 4-digit code to{' '}
            <Text style={styles.resendText2}>{personalEmail}</Text>
        </Text>
        </View>
