...
const handlePreview = async (latestFormValues: any) => {
  try {
    ...
    for (const field of fields) {
      ...
      if (field.mandatory) {
        if (field_type.toLowerCase() === 'date') {
          ...
          return;
        } else if (alias_name?.toLowerCase() === 'price') {
          const priceNumber = Number(String(value).replace(/[^\d.-]/g, ''));
          const minPrice = Math.max(2, Number(min_listing_price) || 0);
          if (isNaN(priceNumber) || priceNumber < minPrice) {
            showToast(
              `${t('price_must_be_at_least')}£${minPrice.toFixed(2)}`,
              'error',
            );
            return;
          }
    ...
    if (priceField) {
      ...
      const priceNumber = Number(String(priceValue ?? '').replace(/[^\d.-]/g, ''));
      const minPrice = Math.max(2, Number(min_listing_price) || 0);
...
//             setMaxFeatureCap(Number(json.metadata.category.max_feature_cap ?? '0'));
//             setaccommodation_amount(Number(json.metadata.category.accommodation_amount ?? '0'));
//             // setminListingPrice(Number(json.metadata.category.accommodation_amount ?? '0'))
//           }
//           setminListingPrice(Number(json.metadata.category.min_listing_price ?? '0'))
//           setUserMeta({
//             firstname: json.metadata.firstname ?? null,
...
//                   showToast(`${t('price_must_be_at_least')}${2} `, 'error');
//                   return;
//           }
//     }
