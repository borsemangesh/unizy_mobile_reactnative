...
const EditProfile = ({ navigation }: EditProfileProps) => {
  ...
  // When postal code changes, keep Save disabled until lookup succeeds
...
//   }
  ...
  const ClickPostalCode = async (postalCode: any) => {
  const lookupId = ++postalLookupIdRef.current;
  const location = await getCityFromPostalCode(postalCode);

  // Ignore stale responses from an older postal-code lookup
  if (lookupId !== postalLookupIdRef.current) {
    return;
  }
    ...
    <ImageBackground
      ...
      <View style={styles.fullScreenContainer}>
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  ...
                  onChangeText={text => {
                    ...
                    const initialPostal = (initialProfile?.postal_code || '')
                      .replace(/\s+/g, '')
                      .toUpperCase();
                    const isSameAsInitial = filteredText === initialPostal;

                    setUserMeta(prev => ({
                      ...prev,
                      postal_code: filteredText,
                      // Clear city while a new code is being entered so Save can't use a stale city
                      ...(isSameAsInitial
                        ? {
                            city: initialProfile?.city || '',
                            latitude: initialProfile?.latitude ?? 0,
                            longitude: initialProfile?.longitude ?? 0,
                          }
                        : {
                            city: '',
                            latitude: 0,
                    ...
                    // Same as saved profile → treat as valid; otherwise wait for lookup
                    if (isSameAsInitial) {
                      setIsPostalCodeValid(true);
                      postalLookupIdRef.current += 1; // cancel in-flight lookups
        ...
        <SaveButton
          ...
          disabled={!isProfileChanged() || !isPostalCodeValid}
        />
      </View>
