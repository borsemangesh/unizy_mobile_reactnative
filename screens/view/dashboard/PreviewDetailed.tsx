...
//     const createRes = await fetch(`${MAIN_URL.baseUrl}category/featurelist/create`, {
//       method: 'POST',
//       headers: {
...
//         console.log(`Step 7: Uploading image ${image.name} with param_id=${param_id} to ${uploadUrl}`);
...
const handleListPress = async () => {
  ...
  try {
    ...
    // 3️⃣ Separate image and non-image fields
    const entries = Object.entries(formData) as [string, any][];

    const imageFields: [string, ImageField[]][] = entries.filter(
      ([, value]): value is ImageField[] =>
        Array.isArray(value) && value.every((v) => v?.uri)
    );

    const nonImageFields: [string, any][] = entries.filter(
      ([, value]) => !(Array.isArray(value) && value.every((v) => v?.uri))
    );

    // 4️⃣ Prepare non-image data for API
    const dataArray = nonImageFields.map(([key, value]) => ({
      id: Number(key),
      param_value: value,
    }));
    ...
    >
      ...
      {/* {storedForm?.[6]?.length > 1 ? (
          <View>
            ...
            >
              {storedForm[6].map((_: any, index: Key | null | undefined) => (
                <View
                  key={index}
                  style={{
                    height: 8,
  ...
  />
          ...
          <Text style={styles.productDesc}>
              ...
              <View style={styles.datePosted}>
                ...
                <Text style={styles.userSub}>Date Posted: {currentDate}</Text>
          ...
          <View style={styles.card}>
            <View style={styles.gap12}>
              ...
              {storedForm?.[9]?.map((id: number, index: Key | null | undefined) => {
                ...
                <View style={styles.gap4}>
                  ...
                  <View style={styles.categoryContainer}>
                {storedForm?.[10]?.map((id: number, index: Key | null | undefined) => {
