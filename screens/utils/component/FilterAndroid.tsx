...
const FilterAndroid = ({
  ...
  const [priceRange, setPriceRange] = useState({ min: 0, max: 10000 });
  // Keep as strings so clearing the field does not auto-fill "0" while typing
  const [sliderLow, setSliderLow] = useState(String(priceRange.min));
  const [sliderHigh, setSliderHigh] = useState(String(priceRange.max));
      ...
      if (data.statusCode === 200) {
        ...
        if (priceFilter) {
          ...
          if (!lastAppliedPriceRange) {
            ...
            setSliderLow(String(newRange.min));
            setSliderHigh(String(newRange.max));
          }
        }
    ...
    if (visible && initialFilters?.filters?.length > 0) {
      ...
      initialFilters.filters.forEach((f: any) => {
        ...
        if (
          ...
          setSliderLow(String(minVal));
          setSliderHigh(String(maxVal));
          setLastAppliedPriceRange({ min: minVal, max: maxVal }); 
        }
      });
  ...
  const handleClearFilters = () => {
    ...
    setSliderLow(String(defaultPriceRange.min));
    setSliderHigh(String(defaultPriceRange.max));
  ...
  const renderRightContent = () => {
    ...
    } else if (currentFilter.alias_name?.toLowerCase() === 'price') {
      return (
        <View
              ...
              <View style={{ flex: 1, marginRight: 5 }}>
                ...
                <TextInput
                  ...
                  onChangeText={text => {
                    ...
                    // Keep empty while typing — do not auto-replace with 0
                    setSliderLow(cleaned);
                    ...
                    const maxVal =
                      sliderHigh === ''
                        ? priceRange.max
                        : parseInt(sliderHigh, 10);

                    setPriceRange({ min: value, max: maxVal });
                  }}
                />
              </View>
                ...
                <TextInput
                  ...
                  onChangeText={text => {
                    ...
                    const minVal =
                      sliderLow === ''
                        ? priceRange.min
                        : parseInt(sliderLow, 10);

                    setPriceRange({ min: minVal, max: value });
                  }}
                />
              </View>
            </View>
  ...
  const validatePriceRange = () => {
    ...
    const minValue = parseInt(sliderLow, 10);
    const maxValue = parseInt(sliderHigh, 10);
  ...
  const handleApply = () => {
    ...
    const appliedMin = parseInt(sliderLow, 10);
    const appliedMax = parseInt(sliderHigh, 10);
    const appliedPriceRange = { min: appliedMin, max: appliedMax };
      ...
      .map(f => {
        if (f.field_type === 'dropdown' && dropdownSelections[f.id]?.length) {
          ...
          return {
            ...
            field_type: f.field_type,
            alias_name: f.alias_name,
            options: [appliedPriceRange.min, appliedPriceRange.max],
          };
    ...
    onApply(filterBody);
    setLastAppliedPriceRange(appliedPriceRange);
    onClose();
  };
